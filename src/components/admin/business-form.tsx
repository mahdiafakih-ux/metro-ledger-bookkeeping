...
export function BusinessForm({ businessId, initial, pricingPlans }: { businessId?: string; initial?: Partial<BusinessInput>; pricingPlans: { key: string; name: string }[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const input: BusinessInput = {
      companyName, category, contactName, email, phone, packageKey,
      monthlyUsage, monthlyRevenueDollars: monthlyRevenue, status,
      contractStart: contractStart || undefined, renewalDate: renewalDate || undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <FormField label="Contact Person"><Input value={contactName} onChange={(e) => setContactName(e.target.value)} /></FormField>
        ...
        <FormField label="Monthly Usage (appointments)"><Input type="number" min={0} value={monthlyUsage} onChange={(e) => setMonthlyUsage(Number(e.target.value))} /></FormField>
